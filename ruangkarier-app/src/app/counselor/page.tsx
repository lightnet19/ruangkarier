...
export default function CounselorDashboard() {
  ...
  const handleSeedMockData = () => {
    ...
    alert("Berhasil menambahkan 3 contoh data simulasi siswa BK dengan profil yang sangat realistis!");
  };

  // Clear all data
  const handleClearAllData = () => {
    if (confirm("Apakah Anda yakin ingin menghapus seluruh data siswa di dasbor ini? Tindakan ini tidak dapat dibatalkan.")) {
      setSessions([]);
  ...
  const handleDeleteSession = (id: string) => {
    if (confirm("Hapus data pengerjaan siswa ini?")) {
